...
public partial class App : AppBase, IAppHost
{
    ...
    {
        if (popup.Child is not { Width: 0, Height: 0 } child ||
            TopLevel.GetTopLevel(child) is not WindowBase topLevel)
        {
            return;
        }
...
#if RELEASE
        // Apple 移动端由 AppDelegate 按 DEVELOPER_PREVIEW 编译常量显示提示，
        // 避免正式 iOS Release 误报或预览构建重复弹窗。
