...
/// <summary>
/// Validates app-owned configuration in a staged import before live data is changed.
/// Plugin-owned and unknown files intentionally remain opaque.
/// </summary>
internal static class AppDataConfigurationValidator
{
    ...
    private static void ValidateKnownConfigFiles(string configRoot)
    {
                     ...
                     Path.Combine(configRoot, "Automations")))
        {
            _ = Deserialize<ObservableCollection<Workflow>>(
                ...
                (workflows, candidate) =>
                {
                    if (workflows.Any(x => x == null))
                    {
                        throw InvalidRequiredValue(candidate, "workflow entry");
                    }
                });
        }
        ...
        Action<T, string>? validate = null)
    {
        return StagedDataImportValidator.LoadPrimaryOrBackup(
            ...
            candidate =>
            {
                var value = DeserializeCandidate<T>(candidate, expectedRootKind);
        ...
        JsonValueKind expectedRootKind)
    {
        ...
        try
        {
            using var input = new FileStream(
                ...
                FileAccess.Read,
                FileShare.Read);
            return ConfigureFileHelper.LoadConfigUnWrapped<T>(input);
        }
