...
internal static class StagedDataImportValidator
{
        ...
        JsonValueKind? expectedRootKind)
    {
        ...
        {
            throw new InvalidDataException($"无法读取 JSON 配置：{path}", exception);
        }
    }
        ...
        Action<string> validateCandidate)
    {
        ...
        {
            throw new InvalidDataException("数据文件中不包含有效的档案 JSON 文件。");
        }
    ...
    private static (bool Success, T? Value, InvalidDataException? Error) TryLoad<T>(
        string path,
        Func<string, T> loadAndValidate)
    {
        if (!File.Exists(path))
    ...
    private static void PromoteBackup(string backupPath, string primaryPath)
    {
        ...
        finally
        {
            ...
            {
                // The staged import cleanup will remove a temporary file left behind here.
            }
