...
{
    [Fact]
    public void Create_PreservesChainOrderAndRemovesDuplicateText()
    {
    ...
    [Fact]
    public void Create_UsesProviderAsTitleWhenOnlyOverlayHasText()
    {
        var payload = IosFallbackNotificationPayloadPolicy.Create(
            "天气提醒",
            [new(null, "今日有雨")]);
    ...
    [Fact]
    public void Create_UsesProviderAsBodyWhenItDiffersFromContentTitle()
    {
        var payload = IosFallbackNotificationPayloadPolicy.Create(
            "自动化",
            [new("任务完成", "任务完成")]);

        Assert.Equal("任务完成", payload.Title);
        Assert.Equal("自动化", payload.Body);
    }

    [Fact]
    public void Create_UsesGenericBodyWhenOnlyProviderIsAvailable()
    {
        var payload = IosFallbackNotificationPayloadPolicy.Create(
            "\0 管理提醒 \0",
            [new(" ", null)]);

        Assert.Equal("管理提醒", payload.Title);
        Assert.Equal("你有一条新提醒。", payload.Body);
    }

    [Fact]
    public void Create_UsesOverlayAsTitleWhenProviderAndMaskAreUnavailable()
    {
        var payload = IosFallbackNotificationPayloadPolicy.Create(
            null,
            [new(null, "仅有正文")]);

        Assert.Equal("仅有正文", payload.Title);
        Assert.Equal("你有一条新提醒。", payload.Body);
    }

    [Fact]
    public void Create_UsesGenericPayloadWhenNoTextIsAvailable()
    {
        var payload = IosFallbackNotificationPayloadPolicy.Create(
            "\0",
            Array.Empty<IosFallbackNotificationTextEntry>());

        Assert.Equal("ClassIsland 提醒", payload.Title);
        Assert.Equal("你有一条新提醒。", payload.Body);
    }
